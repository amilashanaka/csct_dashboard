...
def shape_input():
  ...
  X_train = []
  y_train = []
  for i in range(32, len(train_data)):
      X_train.append(train_data[i - 32 : i, 0])
...
def start():
  ...
  df = data_set.sort_values('Date', ascending=True)
  ...
  test_data = scaled_data[train_close_len - 32 : , :]
  # create X_test and y_test
  X_test = []
  y_test = df.iloc[train_close_len : , :]
  for i in range(32, len(test_data)):
      X_test.append(test_data[i - 32 : i, 0])
...
def tranning():
  global tranning_data_set
  tranning_data_set=tranning_data_set.set_index(tranning_data_set['Date'])
  return tranning_data_set.to_json(orient='records')
